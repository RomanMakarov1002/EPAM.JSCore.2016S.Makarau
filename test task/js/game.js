...
var pumpkinsCount=0;
var cheeseCount=0;
var resources = ['cherry', 'orange', 'pumpkin', 'cheese'];
	...
	function addCounter(){
		...
		if ($this)
		{
			if ($this.hasClass('cherry')){
				$('.cherry p').html(counters[0]+=1);
			}
			else if ($this.hasClass('orange')){
				$('.orange p').html(counters[1]+=1);
			}
			else if ($this.hasClass('pumpkin')){
				$('.pumpkin p').html(counters[2]+=1);
			}
			else if ($this.hasClass('cheese')){
				$('.cheese p').html(counters[3]+=1);
			}
			$this.remove();
		}
	}
